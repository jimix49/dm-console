...
import { Dices, Save, RotateCcw, ShieldAlert, Cpu, UploadCloud } from 'lucide-react';
...
export default function Header({ encounterName, setEncounterName, clearEncounter, onExport, onImport, diceOpen, setDiceOpen }: HeaderProps) {
  ...
  const handleSaveSnapshot = () => {
    onExport();
    toast.success('Combat download started.', {
    ...
    <header className="h-14 border-b border-border/50 bg-card/90 backdrop-blur-md flex items-center justify-between px-4 z-20 shrink-0 sticky top-0">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <Button variant="ghost" size="sm" onClick={handleSaveSnapshot} className="h-8 font-mono text-xs text-muted-foreground" data-testid="button-save-encounter">
          <Save className="w-3.5 h-3.5 mr-2" /> <span className="hidden sm:inline">Download</span>
        </Button>
        <Button variant="ghost" size="sm" onClick={handleImportClick} className="h-8 font-mono text-xs text-muted-foreground" data-testid="button-import-encounter">
          <UploadCloud className="w-3.5 h-3.5 mr-2" /> <span className="hidden sm:inline">Import</span>
        </Button>
