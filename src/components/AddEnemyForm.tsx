  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="bg-primary/20 text-primary border border-primary/50 font-mono uppercase tracking-wider text-xs h-6 px-2" data-testid="button-new-target">
          <Plus className="w-3 h-3 mr-1" /> New Foe
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] bg-card border-border medieval-panel text-card-foreground">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 pt-4">
          <div className="space-y-1.5">
            ...
            <Input id="name" value={name} onChange={e => setName(e.target.value)} required className="bg-card/60 border-secondary/30 font-mono focus-visible:ring-primary h-8 text-sm" placeholder="Goblin" data-testid="input-enemy-name" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="hp" className="text-xs text-muted-foreground uppercase font-mono">Health</Label>
              <Input id="hp" type="number" min="1" value={hp} onChange={e => setHp(e.target.value)} required className="bg-card/60 border-primary/30 font-mono text-center h-8 text-sm" placeholder="15" data-testid="input-enemy-hp" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="ac" className="text-xs text-muted-foreground uppercase font-mono">Armor Class</Label>
              <Input id="ac" type="number" min="1" value={ac} onChange={e => setAc(e.target.value)} required className="bg-card/60 border-primary/30 font-mono text-center h-8 text-sm" placeholder="12" data-testid="input-enemy-ac" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="init" className="text-xs text-muted-foreground uppercase font-mono">Initiative (Optional)</Label>
            <Input id="init" type="number" value={init} onChange={e => setInit(e.target.value)} className="bg-card/60 border-primary/30 font-mono text-center h-8 text-sm" placeholder="18" data-testid="input-enemy-init" />
          </div>
          ...
          <Button type="submit" className="w-full bg-primary text-primary-foreground font-mono uppercase tracking-widest h-8 text-sm" data-testid="button-submit-target">Enlist</Button>
        </form>
      </DialogContent>
    </Dialog>
  );
